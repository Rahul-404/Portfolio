...
function getDemoButtonHTML(project) {
  ...
  // Default to standard Live Demo button
  return `
    <a href="${project.links.demoUrl}" class="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 ${project.theme.primaryBtnClass} font-medium text-xs rounded-lg transition border cursor-pointer">
      <svg class="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
      <span>Live Demo</span>
  ...
  if (!grid) return;
    ...
    grid.innerHTML = projects.map(project => `
        ...
        <div class="project-card relative p-[1px] rounded-2xl overflow-hidden bg-slate-950/95 border border-slate-800/80 transition-all duration-300 h-full flex flex-col">
            ...
            <!-- Radial Shadow Backdrop Follower Glow -->
            ...
            <!-- Vector Border Ring Tracker Edge -->
            ...
            <!-- Top content alignment node container -->
            <div>
            ...
            <!-- Bottom Actions Section - Fixed to base bounds -->
...
window.filterCategory = function(selectedButton, categoryKey) {
  ...
  allTabs.forEach(btn => {
    ...
    if (badge) badge.classList.replace('text-slate-400', 'text-slate-500');
  });
  ...
  if (activeBadge) activeBadge.classList.replace('text-slate-500', 'text-slate-400');
...
// Initial Boot Trigger Execution
...
document.querySelectorAll('#menu-links a').forEach(link => {
  link.addEventListener('click', () => {
    if (menuLinks && !menuLinks.classList.contains('hidden')) {
...
// Set initial state styling on page load configuration
document.addEventListener("DOMContentLoaded", () => {
  const allButton = document.querySelector('[data-filter="all"]');
  if(allButton) allButton.classList.add('tab-active-gradient');
});
 ...
 * Attached directly to 'window' so inline HTML onclick handlers can see it.
 */
window.filterCategory = function(selectedButton, categoryKey) {
    ...
    allTabs.forEach(btn => {
        ...
        // Reset the inner project counter badge color state
        const badge = btn.querySelector('.font-mono');
        if (badge) {
    ...
    // 2. Activate the clicked target tab
    selectedButton.classList.add('tab-active-gradient', 'text-white');
    selectedButton.classList.remove('text-slate-400');

    // Highlight the project count badge for selected focus node
    const activeBadge = selectedButton.querySelector('.font-mono');
    if (activeBadge) {
        activeBadge.classList.replace('text-slate-500', 'text-slate-400');
    }

    // 3. ACTIVATED: Card Filtering Loop Execution
    console.log(`Filtering showcase components to display: ${categoryKey}`);

    // Selects your project cards. 
    // NOTE: Make sure your card HTML elements contain the class 'project-card-wrapper'
    const projectCards = document.querySelectorAll('.project-card-wrapper');
    
    projectCards.forEach(card => {
      // Reads the data-category attribute from your cards (e.g., data-category="genai")
      const cardCategory = card.getAttribute('data-category');
...
// Mouse movement radial neon grid trace tracking
document.querySelectorAll('.project-card').forEach((card) => {
  card.addEventListener('mousemove', (e) => {
    const rect = card.getBoundingClientRect();
    const x = e.clientX - rect.left; 
    ...
    const glows = card.querySelectorAll('.opacity-0');
    glows.forEach(glow => glow.style.opacity = '1');
  });

  card.addEventListener('mouseleave', () => {
    card.querySelectorAll('.pointer-events-none').forEach(g => g.style.opacity = '0');
  });
});